...
description: Capture one mechanism from the current thread and create a single Obsidian Mechanism Card in seed state. Use when the user wants to turn an explanation of why something works, what variables matter, or how a causal chain operates into one mechanism-focused learning card, save it under the resolved vault root at 学习/Cards/Mechanisms, preserve source cues from the thread, and add conservative backlinks to existing cards.
---
...
Create exactly one Mechanism Card from the current thread. This skill is intentionally narrow: it does not create concept, method, or misconception cards, and it does not create multiple mechanisms in one pass.

Read [references/mechanism-card-spec.md](references/mechanism-card-spec.md), [references/vault-path-resolution.md](references/vault-path-resolution.md), and [references/render-script.md](references/render-script.md) before creating or updating any card.
...
- the content is mainly defining a concept rather than explaining a causal process
...
7. If the card does not exist, create one new Mechanism Card with `status: seed`.
8. Fill the card from the current thread, preferring concise synthesis over long transcript dumps.
9. When practical, render the final markdown with `scripts/render_mechanism_card.py` for deterministic structure.
...
- Keep the card focused on the explained phenomenon, core variables, causal chain, prerequisites, weak points, alternative explanations, scope, boundaries, evidence, anomalies, and compression.
