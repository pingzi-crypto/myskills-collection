...
description: Capture, update, or promote one Obsidian Method Card from the current thread. Use when the user wants to turn a practical way of doing something, a procedure, or an actionable strategy into one method-focused learning card, or when an existing Method Card should gain stronger progression evidence, local graph structure, or dispatch rules under the resolved vault root at 学习/Cards/Methods.
---
...
Create, update, or promote exactly one Method Card from the current thread.
This skill is intentionally narrow: it does not create concept, mechanism, or
misconception cards, and it does not create multiple methods in one pass.
...
- [references/method-card-spec.md](references/method-card-spec.md)
...
- the output should stay within one Method Card boundary
...
1. Resolve `VAULT_ROOT` using [../shared/learning-card-core/references/vault-path-protocol.md](../shared/learning-card-core/references/vault-path-protocol.md) and the target paths in [references/method-card-spec.md](references/method-card-spec.md).
...
7. If the card does not exist, create one new Method Card with `status: seed` and `graph_maturity: none`.
...
10. When practical, render the final markdown with `scripts/render_method_card.py` for deterministic structure.
...
- mature an existing Method Card
- decide whether a Method Card should move beyond `stable`
- add stronger knowledge-graph structure or dispatch rules
...
- Prefer `scripts/render_method_card.py` to generate the final card layout for new cards, following [../shared/learning-card-core/references/deterministic-render-protocol.md](../shared/learning-card-core/references/deterministic-render-protocol.md).
